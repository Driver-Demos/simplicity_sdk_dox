...
/// @brief Type of frames that can be used to transport system endpoint data
...
/***************************************************************************//**
 * The biggest property payload that the system endpoint will ever send. This is
 * the response to the get all endpoint states command. Each byte can hold two
 * endpoint states, hence the divide by two at the end of the macro.
 ******************************************************************************/
#define SLI_CPC_PROPERTY_PAYLOAD_LENGTH_MAX (SL_CPC_ENDPOINT_MAX_COUNT * sizeof(sl_cpc_endpoint_state_t) / 2)

/***************************************************************************//**
 * The biggest payload that the system endpoint will ever send is the biggest
 * payload plus the property id that corresponds to that payload.
 ******************************************************************************/
#define SLI_CPC_SYSTEM_COMMAND_PAYLOAD_LENGTH_MAX (sizeof(sli_cpc_system_property_cmd_t) \
                                                   + SLI_CPC_PROPERTY_PAYLOAD_LENGTH_MAX)

/***************************************************************************//**
 * Forward declaration of sli_cpc_system_endpoint_t to make it known to
