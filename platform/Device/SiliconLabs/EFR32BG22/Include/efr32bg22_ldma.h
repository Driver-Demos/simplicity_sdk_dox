...
/***************************************************************************//**
 * @brief The `LDMA_CH_TypeDef` structure defines the layout of a channel in the
 * Linked Direct Memory Access (LDMA) controller for the EFR32BG22
 * microcontroller. It includes configuration, control, source,
 * destination, and link address registers, as well as reserved spaces
 * for future expansion. This structure is used to manage and configure
 * individual DMA channels, allowing for efficient data transfer
 * operations between memory and peripherals.
 *
 * @param RESERVED0 An array reserved for future use.
 ...
 * @param RESERVED1 An array reserved for future use.
 ******************************************************************************/
 ...
 * registers for the Low-Density Memory Access (LDMA) controller in the
 * EFR32BG22 microcontroller. It includes registers for enabling and
 * disabling the DMA module, controlling DMA operations, monitoring
 * status, and handling synchronization and channel operations. The
 * structure also provides set, clear, and toggle versions of these
 * registers to facilitate atomic operations. Additionally, it contains
 * an array of channel-specific registers (`CH`) for managing up to 8 DMA
 * channels, each with its own configuration and control settings.
